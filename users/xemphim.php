...
$videoSource = !empty($movie["link1"]) ? trim((string) $movie["link1"]) : "";
if ($videoSource === "" && !empty($movie["link2"])) {
    $videoSource = trim((string) $movie["link2"]);
}
...
<div class="videophim">
    <?php if ($videoSource !== "") { ?>
        <iframe class="video" width="800" height="500" src="https://www.youtube.com/embed/<?php echo htmlspecialchars($videoSource); ?>" frameborder="0" allowfullscreen></iframe>
    <?php } else { ?>
        <div class="movie-detail-empty">
            <h1>Phim chưa có nguồn phát</h1>
            <p>Admin chưa cấu hình server xem phim cho nội dung này.</p>
        </div>
    <?php } ?>
</div>
...
<div class='pagination'>
    <?php if ($currentPage > 1) { ?>
        <a href='?page_layout=xemphim&id=<?php echo $ID; ?>&page=<?php echo $currentPage - 1; ?>'><img src='anhnen/previous.png' alt='Back' style='width: 20px; height: 20px;'></a>
    <?php } ?>
    ...
    <?php if ($currentPage < $totalPages) { ?>
        <a href='?page_layout=xemphim&id=<?php echo $ID; ?>&page=<?php echo $currentPage + 1; ?>'><img src='anhnen/next.png' alt='Next' style='width: 20px; height: 20px;'></a>
    <?php } ?>
</div>
